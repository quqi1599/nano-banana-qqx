                    ...
                    <table className="w-full text-left text-sm border-collapse">
                        <thead className="sticky top-0 z-10 bg-gray-50 dark:bg-gray-800 shadow-sm">
                            <tr className="text-xs uppercase tracking-wider text-gray-500 dark:text-gray-400">
                                <th className="px-4 py-3.5 font-semibold min-w-[180px] lg:min-w-[200px]">名称 / Key</th>
                                <th className="px-4 py-3.5 font-semibold whitespace-nowrap">状态</th>
                                ...
                                <th className="px-4 py-3.5 font-semibold min-w-[180px] lg:min-w-[220px]">Base URL</th>
                                <th className="px-4 py-3.5 font-semibold whitespace-nowrap">
                                    <button
                                        type="button"
                                ...
                                <th className="px-4 py-3.5 font-semibold text-center whitespace-nowrap">请求数</th>
                                <th className="px-4 py-3.5 font-semibold text-right whitespace-nowrap min-w-[130px]">操作</th>
                            </tr>
                        </thead>
                                ...
                                return (
                                    <tr key={token.id} className="hover:bg-gray-50/60 dark:hover:bg-gray-800/60 transition-colors">
                                        <td className="px-4 py-3.5 align-top">
                                            <div className="font-semibold text-gray-900 dark:text-gray-100 mb-1.5 truncate">{token.name}</div>
                                            <div className="flex items-center gap-1.5 flex-wrap">
                                                <code className="font-mono text-[11px] bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded text-gray-600 dark:text-gray-400 break-all max-w-[120px] lg:max-w-[140px]">
                                                    {displayKey}
                                                </code>
                                                <button
                                        ...
                                        <td className="px-4 py-3.5 text-center">
                                            <span className="inline-block text-xs font-mono bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded-md text-gray-600 dark:text-gray-300">
                                                {token.priority}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-4 py-3.5">
                                            <div className="flex items-center gap-1.5">
                                                <input
                                                    ...
                                                    placeholder="默认端点"
                                                    className="flex-1 min-w-[140px] lg:min-w-[160px] px-2.5 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 dark:bg-gray-800 text-sm focus:ring-2 focus:ring-cream-500 outline-none transition"
                                                />
                                                {baseUrlDirty && (
                                                    <button
                                                        onClick={() => onSaveBaseUrl(token.id)}
                                                        disabled={savingTokenUrl[token.id]}
                                                        className="p-1.5 rounded-lg bg-cream-100 text-cream-700 hover:bg-cream-200 dark:bg-cream-900/30 dark:text-cream-300 transition flex-shrink-0"
                                                        title="保存"
                                                    >
                                                        {savingTokenUrl[token.id] ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Check className="w-3.5 h-3.5" />}
                                                    </button>
                                                )}
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-4 py-3.5 text-center font-mono text-sm text-gray-600 dark:text-gray-400">
                                        ...
                                        <td className="px-4 py-3.5">
                                            <div className="flex items-center justify-end gap-0.5">
                                                ...
                                                <button
                                                    ...
                                                    className={`p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition ${token.is_active ? 'text-gray-400 hover:text-red-500' : 'text-emerald-600 hover:text-emerald-700'}`}
