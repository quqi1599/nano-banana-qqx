          ...
          {dialog.progress !== undefined && (
            <div className="w-full mb-4">
              <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                <span>{dialog.progressLabel || '进度'}</span>
                <span>{Math.round(dialog.progress * 100)}%</span>
              </div>
              <div className="mt-1 h-2 w-full rounded-full bg-gray-100 dark:bg-gray-800 overflow-hidden">
                <div
                  className={`h-2 rounded-full ${dialog.type === 'alert' ? 'bg-amber-500' : 'bg-red-500'}`}
                  style={{
                    width: `${Math.min(100, Math.max(4, Math.round(dialog.progress * 100)))}%`,
                  }}
                />
              </div>
            </div>
