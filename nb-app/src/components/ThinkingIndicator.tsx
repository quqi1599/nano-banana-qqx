    ...
    return (
        <div className={`flex w-full justify-center py-6 ${isExiting ? 'fade-out-down' : 'fade-in-up'}`}>
            <div className="relative w-full max-w-xl group">
                ...
                <div className={`relative flex flex-col items-center backdrop-blur-md rounded-xl shadow-2xl border overflow-hidden transition-colors duration-300 ${isDark
                        ? 'bg-gray-950/90 border-gray-800'
                        : 'bg-white/90 border-gray-200'
                    }`}>

                    {/* Header Status Bar */}
                    <div className={`flex w-full items-center justify-between px-4 py-3 border-b transition-colors duration-300 ${isDark ? 'bg-gray-900/50 border-gray-800' : 'bg-gray-50/80 border-gray-200'
                        }`}>
                        ...
                        <div className="flex items-center gap-3">
                            <div className={`flex items-center gap-1.5 px-2 py-1 rounded border ${isDark ? 'bg-gray-800/50 border-gray-700/50' : 'bg-white border-gray-200'
                                }`}>
                                <BrainCircuit className={`h-3 w-3 ${isDark ? 'text-cream-400' : 'text-cream-500'}`} />
                            ...
                            {onClose && (
                                <button
                                    onClick={onClose}
                                    className={`p-1 rounded-full transition-colors ${isDark ? 'hover:bg-gray-800 text-gray-500 hover:text-gray-300' : 'hover:bg-gray-200 text-gray-400 hover:text-gray-600'
                                        }`}
                                    title="关闭 Arcade"
                                >
                                    <X className="h-4 w-4" />
                    ...
                    {/* Game Container */}
                    <div className={`w-full p-4 flex flex-col items-center justify-center min-h-80 transition-colors duration-300 ${isDark ? 'bg-gray-950' : 'bg-gray-100'
                        }`}>
                    ...
                    {/* Footer */}
                    <div className={`w-full py-2 border-t text-center transition-colors duration-300 ${isDark ? 'bg-gray-900/50 border-gray-800' : 'bg-gray-50/80 border-gray-200'
                        }`}>
